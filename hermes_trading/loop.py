...
class TradingLoop:
    ...
    async def open_trade(self, asset: str, side: str, price: float, tf_data: dict, q_scores: dict):
        self._positions[asset] = {
            ...
            "indicators": {
                "signal_asset": asset,
                "rsi_4h": (tf_data.get("4h") or {}).get("rsi", 0),
                "rsi_1h": (tf_data.get("1h") or {}).get("rsi", 0),
                "rsi_15m": (tf_data.get("15m") or {}).get("rsi", 0),
                "ema50_4h": (tf_data.get("4h") or {}).get("ema50", 0),
                "ema200_4h": (tf_data.get("4h") or {}).get("ema200", 0),
                "ema50_1h": (tf_data.get("1h") or {}).get("ema50", 0),
                "volume_ratio": tf_data.get("1h", {}).get("volume", 1) / max(tf_data.get("1h", {}).get("vol_avg", 1), 1),
    ...
    async def close_trade(
        ...
        trade = {
            ...
            "indicators": {
                "entry": entry_ind,
                "exit": {
                    "rsi_4h": (tf_data.get("4h") or {}).get("rsi", entry_ind.get("rsi_4h", 0)),
                    "rsi_1h": (tf_data.get("1h") or {}).get("rsi", entry_ind.get("rsi_1h", 0)),
                    "rsi_15m": (tf_data.get("15m") or {}).get("rsi", entry_ind.get("rsi_15m", 0)),
                    "close_4h": (tf_data.get("4h") or {}).get("close", entry_ind.get("close_4h", 0)),
                    "close_1h": (tf_data.get("1h") or {}).get("close", entry_ind.get("close_1h", 0)),
                },
            },
